...
class Game:
    ...
    def get_all_client_data(self) -> dict:
        ...
        return {
            "game-state": {
                player: self.get_client_data(player) for player in self.players
            }
        }
    ...
    def is_valid_playercount(self) -> bool:
        ...
        """
        return (
            self.player_constraints[0] <= self.num_players <= self.player_constraints[1]
        )
...
def create_game(game_id: str, players: [str]) -> Optional[Game]:
    ...
    return (
        r_val,
        "Not enough players to start the game. Get more friends and then retry 🤓"
        if not r_val
        else None,
    )
