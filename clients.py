...
class Clients:
            ...
            if self.is_party(party_id) and self.is_in_party(client_id, party_id):
                ...
                if game_id in self.valid_games:
                    if self.is_game_playing(party_id):
                        ...
                        game, err = cg(game_id, [client for client in self.parties[party_id]])
                        if game is not None:
        ...
        if self.is_user(client_id):
            ...
            if self.is_party(party_id) and self.is_in_party(client_id, party_id):
                ...
                if self.is_game_playing(party_id):
                    ...
                    if has_won:
                        print("Game is over!")
                        print(f"Game: {won_status}")
                        return True, won_status["game-state"][client_id]
    ...
    def delete_game(self, party_id: str, client_id: str):
        if self.is_user(client_id):
            print("Checking if user is in correct party...")
            if self.is_party(party_id) and self.is_leader(client_id, party_id):
...
def make_random_not_in_list(list: list, length: int = 2):
